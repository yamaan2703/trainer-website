...
export function CtaLink({ href, children, variant = "solid", className }: CtaLinkProps) {
  ...
  return (
    <MotionAppLink
      ...
      className={cn(
        "group inline-flex w-fit items-center gap-3 px-7 py-4 text-sm font-semibold uppercase tracking-[0.08em]",
        variant === "solid" && "btn-cta",
        variant === "outline" &&
          "border border-hairline text-ink transition-colors hover:border-orange-600 hover:text-orange-600",
        className
      )}
    >
      {children}
